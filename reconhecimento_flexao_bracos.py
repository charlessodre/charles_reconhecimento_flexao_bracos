...
gravar_video = cv2.VideoWriter(video_saida, cv2.VideoWriter_fourcc(*'XVID'), 10, (frame_inicial.shape[1], frame_inicial.shape[0]))
...
def desenha_tracos(pontos, pares_pontos):
    for par in pares_pontos:
        ...
        if pontos[inicio_traco] and pontos[fim_traco]:
            cv2.line(frame, pontos[inicio_traco], pontos[fim_traco], (192, 192, 192), 2, lineType=cv2.LINE_AA)
            cv2.line(video_copia, pontos[inicio_traco], pontos[fim_traco], (192, 192, 192), 2, lineType=cv2.LINE_AA)
        ...
        if percentual_movimento_flexao == 0.75 and y_cabeca_final < y_linha_base_pulso and y_cabeca_final < linha_base_cotovelo:
            ...
            cv2.putText(frame_saida,
                        "Movimento {:.0%} concluido".format(percentual_movimento_flexao), (
                            300, 35), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0) )
